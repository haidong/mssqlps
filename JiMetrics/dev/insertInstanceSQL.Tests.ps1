...
Describe "insertInstanceSQL" {
    It "sql2\finance is inactive and sql2\hr is active" {
		...
		$sql | Should Be "EXEC Windows.Instance_Insert 2, 'sql2\FINANCE', 'haidongworks\bogey', 'N'"
		$sql = insertInstanceSQL $SqlInstances[1] 2
		$sql | Should Be "EXEC Windows.Instance_Insert 2, 'sql2\HR', 'haidongworks\bogey', 'Y'"}
    It "sql1 is active" {
		$SqlInstances = getSqlInstanceName("sql1")
		$sql = insertInstanceSQL $SqlInstances 1
		$sql | Should Be "EXEC Windows.Instance_Insert 1, 'sql1', 'haidongworks\bogey', 'Y'"}}